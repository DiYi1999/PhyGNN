...
from model.ours.temporal_block import MLP_dim2
...
class SPS_Model_Phy_wo_BCR(nn.Module):
    def __init__(self, exp_frequency=1,
                 ...
                 BCR_MLP_hidden_dim=128, BCR_MLP_lay_num=2, replace_BCR_with_MLP_or_0='MLP',
                 dropout=0.0, LeakyReLU_slope=0.01):
        super().__init__()
        ...
        if replace_BCR_with_MLP_or_0 == 'MLP':
            self.BCR = Battery_Charging_Regulator(BCR_MLP_hidden_dim=BCR_MLP_hidden_dim,
                                                BCR_MLP_lay_num=BCR_MLP_lay_num,
                                                dropout=dropout,
                                                LeakyReLU_slope=LeakyReLU_slope)
        self.replace_BCR_with_MLP_or_0 = replace_BCR_with_MLP_or_0
    ...
    def forward(self, S_irr_SA, T_SA, theta, Load_Signal):
        ...
        if self.replace_BCR_with_MLP_or_0 == 'MLP':
            I_BAT, Signal_BAT \
                = self.BCR(I_Remain=I_Remain, Charge_Signal=Charge_Signal, I_BAT_Discharge=I_Discharge, SOC=SOC)
        elif self.replace_BCR_with_MLP_or_0 == '0':
            I_BAT = torch.zeros_like(I_Discharge).to(S_irr_SA.device)
            Signal_BAT = torch.zeros_like(I_Discharge).to(S_irr_SA.device)
...
class Solar_Array(nn.Module):
    def __init__(self, V_mp_ref_all=24, I_mp_ref_all=2.5):
        ...
        self.I_red = nn.Parameter(torch.tensor(0.001))
        self.V_red = nn.Parameter(torch.tensor(0.001))
    ...
    def forward(self, S_irr_SA, T_SA, theta):
        ...
        self.n_p = torch.floor((self.I_mp_ref_all + self.I_red) / self.I_mp_ref_one)
        n_p = self.n_p
        self.n_s = torch.floor((self.V_mp_ref_all + self.V_red) / self.V_mp_ref_one)
        n_s = self.n_s
...
class Battery(nn.Module):
    ...
    def forward(self, Signal_BAT, I_BAT, Q_start_init):
        ...
        for i in range(self.app_coff_discharge.shape[0]):
            U_BAT_discharge = U_BAT_discharge + self.app_coff_discharge[i] * Q_now ** (
                        self.app_coff_discharge.shape[0] - 1 - i)
        ...
        T_BAT = - 8.375354868 * I_BAT + 16.10352945 * Q_now - 15.4012349 * U_BAT - 357.3665664
...
class Battery_Charging_Regulator(nn.Module):
    def __init__(self, BCR_MLP_hidden_dim, BCR_MLP_lay_num, dropout, LeakyReLU_slope):
        super().__init__()

        self.mlp_dim2 = MLP_dim2(in_dim=4,
                                 hidden_dim=BCR_MLP_hidden_dim,
                                 layer_num=BCR_MLP_lay_num,
                                 out_dim=2,
                                 dropout=dropout,
                                 LeakyReLU_slope=LeakyReLU_slope
                                 )
    ...
    def forward(self, I_Remain, Charge_Signal, I_BAT_Discharge, SOC):
        X = torch.cat((I_Remain, Charge_Signal, I_BAT_Discharge, SOC), dim=1)
        X = self.mlp_dim2(X)
        I_BAT = X[:, 0:1, :]
        Signal_BAT = X[:, 1:2, :]
